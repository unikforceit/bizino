...
if (!function_exists('bizino_breadcrumbs')) {
    function bizino_breadcrumbs($args = array())
    {
        if (is_front_page()) {
            return;
        }
        ...
        $defaults = array(
            ...
            'home_title' => esc_html__('Home', 'bizino')
        );
        $args = apply_filters('bizino_breadcrumbs_args', wp_parse_args($args, $defaults));
        ...
        */
        ...
        if (is_singular('post')) {
            ...
            $html .= '<li class="active bread-search">' . esc_html__('Search results for: ', 'bizino') . get_search_query() . '</li>';
        } elseif (is_404()) {
            $html .= '<li>' . esc_html__('Error 404', 'bizino') . '</li>';
        } elseif (is_home()) {
