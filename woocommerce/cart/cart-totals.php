...
<div class="row justify-content-end <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<div class="col-md-8 col-lg-7 col-xl-6">
		<h2 class="h4 summary-title"><?php esc_html_e( 'Cart Totals', 'bizino' ); ?></h2>
		<table cellspacing="0" class="shop_table shop_table_responsive cart_totals">
			<tbody>
				<tr>
					<td><?php esc_html_e( 'Subtotal', 'bizino' ); ?></td>
					<td data-title="<?php esc_attr_e( 'Subtotal', 'bizino' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></td>
				</tr>
				...
				<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
					...
					<tr class="shipping">
						<td><?php esc_html_e( 'Shipping', 'bizino' ); ?></td>
						<td data-title="<?php esc_attr_e( 'Shipping', 'bizino' ); ?>"><?php woocommerce_shipping_calculator(); ?></td>
					</tr>
				...
				if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
					...
					if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
						...
						$estimated_text = sprintf( ' <small>' . esc_html__( '(estimated for %s)', 'bizino' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] );
					}
			...
			<tfoot>
				<tr class="order-total">
					<td><?php esc_html_e( 'Order Total', 'bizino' ); ?></td>
					<td data-title="<?php esc_attr_e( 'Total', 'bizino' ); ?>"><?php wc_cart_totals_order_total_html(); ?></td>
				</tr>
			</tfoot>
